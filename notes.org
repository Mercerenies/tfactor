...
* TRAITS AND MODULE PARAMETERS PLAN
 ...
 6. [-] Parameterized modules
    ...
    + [ ] Parameterization
    + [ ] Actualizing functors into modules
    + [ ] Type checking inside of a functor
    + [ ] Ability to have nested functors
    + [ ] Normalize the type names to behave more intuitively
    + [ ] A functor should depend on anything its inner declarations depend on (for module loading)
